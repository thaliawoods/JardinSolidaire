...
// Accept both `targetId` (preferred) and `ownerId` (legacy) + optional `title`
export default function AvailabilityCalendar({ mode, targetId, ownerId, token, title }) {
  ...
  const gardenerAvail = useGardenerAvailability(id, week.fromISO, week.toISO, token);
  const gardenAvail = useGardenAvailability(id, week.fromISO, week.toISO, token);

  const { data, reload, createSlot, deleteSlot } =
  ...
  return (
    <div className="mt-6 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={() =>
              setCursor((d) => new Date(d.getFullYear(), d.getMonth(), d.getDate() - 7))
            }
          >
            ← Semaine -1
          </button>
      ...
      <WeekGrid week={week} events={events} onAdd={handleAdd} onDelete={handleDelete} />

      <div className="text-xs text-gray-500">
        <span className="inline-block w-3 h-3 align-middle bg-green-200 border border-green-300 mr-1" />{' '}
        Libre
        <span className="inline-block w-3 h-3 align-middle bg-rose-200 border border-rose-300 ml-4 mr-1" />{' '}
        Réservé
        <span className="inline-block w-3 h-3 align-middle bg-gray-200 border border-gray-300 ml-4 mr-1" />{' '}
        Indispo
      </div>
    </div>
  );
