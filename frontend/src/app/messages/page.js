  ...
  return {
    ...
    lastMessage: t.lastMessage
      ? {
          ...
          sentAt: t.lastMessage.date_envoi
            ? new Date(t.lastMessage.date_envoi)
            : null,
  ...
  const send = async () => {
    ...
    try {
      ...
      const latest = await fetch(
        `${API_BASE}/api/messages/with/${activePeer}`,
        {
          headers: { Authorization: `Bearer ${token}` },
          cache: "no-store",
        }
      ).then((r) => r.json());
      const msgs = Array.isArray(latest?.messages)
        ? latest.messages.map(normalizeMessage)
        : [];
      setMessages(msgs);
    } catch (e) {
      console.error(e);
      ...
      <div className="min-h-screen bg-white p-6">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-gray-700">
            ...
            <Link className="text-pink-600 underline" href="/login">
              Sign in
            </Link>
          </p>
        </div>
        ...
        <section className="flex-1 bg-emerald-50 border border-emerald-100 rounded-2xl flex flex-col">
          <header className="p-4 border-b border-emerald-100">
            <h2 className="font-semibold">
              {activeThread ? (
                <>
                  Chat with {activeThread.peer?.firstName}{" "}
                  {activeThread.peer?.lastName}
                </>
              ) : (
          ...
          <footer className="p-3 border-t border-emerald-100">
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  activePeer ? "Write a message…" : "Choose a contact"
                }
                disabled={!activePeer}
