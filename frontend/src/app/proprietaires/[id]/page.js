...
export default function ProprioPage({ params }) {
  const { id } = params || {};
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (alive) {
          setError("Impossible de charger le propriétaire. (Affichage d'un exemple)");
          setOwner({
            nom: 'Exemple Proprio',
            avatarUrl: '',
            isOnline: true,
            ...
            surface: null,
            type: '—',
            presentation: 'Texte de présentation (exemple).',
            comments: [],
          });
        }
      } finally {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 flex flex-col">
      <main className="mx-auto w-full max-w-6xl px-4 sm:px-6 py-8 flex-1">
        <h1 className="text-2xl font-bold text-green-800 mb-6">Informations du Proprio</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              <section className="flex items-start gap-4">
                <div
                  ...
                  aria-label="Avatar du proprio"
                >
                  {owner.avatarUrl && (
                    <img src={owner.avatarUrl} alt={owner.nom || 'Avatar'} className="h-full w-full object-cover" />
                  )}
                  {owner.isOnline && (
                    <span
                ...
                <div className="flex flex-col gap-2">
                  <StatBox value={owner.totalReviews ?? 0} label="évaluations" />
                  <StatBox value={`${owner.rating ?? '—'}+★`} label="en note globale" />
                </div>
              </section>

              <section className="lg:col-span-2">
                <Card title="Informations du Proprio">
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
                    <Field label="Nom" value={owner.nom || '—'} />
                    <Field label="Quartier" value={owner.quartier || '—'} />
                    <Field label="Disponibilités" value={owner.disponibilites || '—'} />
                    <Field label="Surface" value={owner.surface ? `${owner.surface} m²` : '—'} />
                    <Field label="Type de jardin" value={owner.type || '—'} />
                  </div>
                </Card>
              </section>
            </div>
