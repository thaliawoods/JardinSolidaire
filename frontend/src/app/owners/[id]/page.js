...
/** Normalize Owner (EN-first; falls back to FR keys) */
function normalizeOwner(raw) {
  if (!raw) return null;
  const en = {
    id: String(raw.id ?? raw.id_proprietaire ?? ''),
    ...
    rating: raw.rating ?? raw.note ?? null,
    district: raw.district ?? raw.quartier ?? '',
    availability: raw.availability ?? raw.disponibilites ?? '',
    area: raw.area ?? raw.surface ?? null,
    kind: raw.kind ?? raw.type ?? '',
    intro: raw.intro ?? raw.presentation ?? '',
    comments: Array.isArray(raw.comments)
      ? raw.comments.map(c => ({
          id: c.id,
          text: c.text ?? c.commentaire ?? '',
          authorName: c.authorName ?? c.author ?? c.auteur ?? '',
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!res.ok) {
          // Fallback to legacy FR route
          res = await fetch(`${API_BASE}/api/proprietaires/${id}`, { cache: 'no-store' });
        }
        ...
        setOwner({
          ...
          area: null,
          kind: '—',
          intro: 'Example introduction text.',
          comments: [],
        });
      } finally {
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              ...
              <section className="flex items-start gap-4">
                <div className="relative h-28 w-28 rounded-full bg-gray-200 overflow-hidden flex-shrink-0">
                  {owner.avatarUrl && (
              ...
              <section className="lg:col-span-2">
                <Card title="Owner details">
                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
                    <Field label="Full name" value={`${owner.firstName} ${owner.lastName}`.trim() || '—'} />
                    <Field label="Neighborhood" value={owner.district || '—'} />
                    <Field label="Availability" value={owner.availability || '—'} />
                    <Field label="Surface" value={owner.area ? `${owner.area} m²` : '—'} />
                    <Field label="Garden type" value={owner.kind || '—'} />
                  </div>
                </Card>
              </section>
            </div>
                    ...
                    <li key={c.id} className="border rounded-lg p-3">
                      ...
                      {(c.authorName || c.createdAt) && (
                        <p className="mt-1 text-xs text-gray-500">
                          {c.authorName ? `by ${c.authorName}` : ''} {c.createdAt ? `• ${new Date(c.createdAt).toLocaleDateString()}` : ''}
                        </p>
                      )}
                    </li>
                  ))}
