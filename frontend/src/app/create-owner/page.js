...
export default function CreateOwnerPage() {
  ...
  // Owner fields your backend accepts (English keys)
  const [form, setForm] = useState({
    firstName: '',
    lastName: '',
    district: '',       // neighborhood
    availability: '',   // free text, e.g., "evenings and weekends"
    area: '',           // number (m²)
    kind: '',           // garden type
    intro: '',          // short intro
  ...
  // Prefill from /api/me if an owner profile already exists (upsert route will update)
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        ...
        // No owner yet or not logged in → leave blank
  ...
  const onSubmit = async (e) => {
    ...
    const payload = {
      ...
      // avatarUrl, isOnline, rating, totalReviews can be added later; backend has defaults
    };
      ...
      {loading ? (
        ...
        <form onSubmit={onSubmit} className="max-w-3xl mx-auto space-y-5">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            >
              {submitting ? 'Saving…' : 'Save my owner profile'}
            </button>
            <Link
              href="/profile"
